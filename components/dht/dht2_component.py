...
def real_dht2_loop(settings, stop_event, device_info):
    ...
    while not stop_event.is_set():
        ...
        if chk == dht.DHTLIB_OK:
            dht2_callback(code, device_info, settings, dht.temperature, dht.humidity)
        
        time.sleep(settings.get('delay', 2))

def run_dht2(settings, threads, stop_event, device_info):
    if settings['simulated']:
        ...
        dht2_thread = threading.Thread(
            ...
            args=(settings['delay'], lambda c, t, h: dht2_callback(c, device_info, settings, t, h), stop_event, code)
        )
        dht2_thread.start()
        threads.append(dht2_thread)
        print("Master Bedroom DHT simulator started")
    else:
        print(f"Starting {device_info['device_name']} real sensor")
        dht2_thread = threading.Thread(target=real_dht2_loop, args=(settings, stop_event, device_info))
        dht2_thread.start()
        threads.append(dht2_thread)
        print("Master Bedroom DHT real sensor started")