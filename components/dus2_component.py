...
def dus2_callback(distance, code, device_info, settings):
    payload = {
        "measurement": "door ultrasonic 2",
...
def run_dus2(settings, threads, stop_event, device_info):
    if settings['simulated']:
        delay = settings['delay']
        code = settings['code']
        print(f'Starting {code} simulator')
        dus1_thread = threading.Thread(target=run_dus2_simulator, args=(delay, lambda d, c: dus2_callback(d, c, device_info, settings), stop_event, code))          
        dus1_thread.start()
            ...
            try:
                while not stop_event.is_set():
                    ...
                    if iter <= max_iter:
                        ...
                        dus2_callback(distance, code, device_info, settings)
